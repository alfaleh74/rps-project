...
function Card({ ...props }: BitCardProps) {
  ...
  return (
    <div
      className={cn(
        "relative border-y-6 border-gray-600 p-0! overflow-hidden",
        className
      )}
    >
      <ShadcnCard
        {...props}
        className={cn(
          "rounded-none border-0 w-full! bg-gray-800/85",
          font !== "normal" && "retro",
          className
        )}
      />
      ...
      <div
        className="absolute inset-0 border-x-6 -mx-1.5 border-gray-600 pointer-events-none"
        aria-hidden="true"
      />
    </div>
  );
